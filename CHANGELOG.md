...
- **`utils/import_pdf_snapshot.py`**: Now loads `SNAPSHOT_DATE`/`POSITIONS` from `utils/pdf_snapshot_data.py` (gitignored) instead of hardcoding them, with `utils/pdf_snapshot_data.py.example` as the tracked template to copy. Raises a clear error if the local data file is missing instead of running with placeholder data.

### Fixed
- **Duplicate position rows when saving the same date's snapshot more than once**: `Database.add_position()` always `INSERT`ed a new row, with no check for an existing position on the same `(name, date, portfolio)`. Since "Entrada Manual", "Atualizar Posições", and "Registrar Contribuição" (via "Registrar Todas") all re-save the full latest snapshot for a date, running more than one of these on the same day duplicated every untouched asset and left a stale row behind for any asset whose value changed between saves. Fixed by making `add_position()` an upsert: it now looks up `(name, date, portfolio)` first and updates the existing row in place if found, otherwise inserts a new one. Cleaned up 27 duplicate rows that had accumulated for 2026-09-14 in `investment_data.db` (kept the value confirmed correct by the user for each asset; a pre-cleanup backup was saved as `investment_data.db.bak_20260914_142528`).
- **Rebalancing "new money" amounts could exceed the actual investment entered (data-integrity bug)**: Two stacking bugs caused the amounts shown in the Rebalanceamento tab to not add up, leading to over-investment recommendations:
  1. **Category-level amounts weren't capped against the shared investment pool**: `PortfolioCalculator.create_rebalancing_plan()` computed each category's `rebalance_amount` as if that category alone could absorb the entire "Novo Investimento" figure, without checking how much was left after other underweight categories. The category comparison table, expander headers, and "Ação recomendada" text all displayed this uncapped figure, while a separate, correctly-capped calculation only existed in the "Sugestão de alocação" text list. Added `AllocationAnalysis.capped_investment_amount`, computed once in `create_rebalancing_plan()` using the same priority-ordered, shared-pool-aware logic as the suggestions list, and now used everywhere the UI shows "how much to invest in this category."
  2. **Per-asset "Distribuição por Meta" table didn't respect even the category budget**: In `_render_asset_level_rebalancing()`, each asset with a per-asset target (`asset_category_targets`) had its investment amount computed independently as `target_value - current_value`, clamped only at zero on the low end — never capped so that all targeted assets together stay within the category's budget. An asset far below its target % (e.g. a newly added/renamed fund with near-zero balance but a large target) could alone demand more than the entire category's budget, and the table would recommend it without warning. Added `PortfolioCalculator.fit_asset_investments_to_budget()`, a pure, testable helper that proportionally scales down per-asset target investments when their sum would exceed the category budget, used by the "Distribuição por Meta" table and both fallback strategies ("Proporcional aos ativos atuais", "Distribuição igual") in `dashboard.py`. Applied the equivalent comparison-table fix to `previdencia.py`.
  - Added `tests/test_calculations.py` (plain-assert script, `python3 tests/test_calculations.py`) reproducing the exact reported scenario (a category with an asset at a 24% target and ~R$0 current balance) and asserting per-asset totals never exceed the category's allotted budget.
